...
| **模型指定** | Body: model (e.g., claude-3-5-sonnet) | URL Path: models/{model}                | 需建立映射表。例如 claude-3-5-sonnet \-\> gemini-3-pro-preview。Claude Code 可能会硬编码某些模型名称，代理需做模糊匹配或默认回退 1。 |
...
| claude\_req.model.contains("opus") {
             "gemini-3-pro-preview".to\_string()
        } else {
             "gemini-3-pro-preview".to\_string() // 使用更快的 Flash 模型作为默认
        };
